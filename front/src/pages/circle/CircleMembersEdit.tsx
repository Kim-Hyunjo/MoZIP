...
const CircleMembersEdit = () => {
  ...
  const masterList = master.map((item) => {
    return <CircleMembersEditmaster propsRole={item} />;
  });

  const submasterList = submaster.map((item) => {
    return <CircleMembersEditSubmaster propsRole={item} />;
  });
  const techList = tech.map((item) => {
    return <CircleMembersEditTech propsRole={item} />;
  });
    ...
    <div className="circleMembersEdit">
      ...
      <div>
        <ul className="membersKind">
          ...
          <li>회계</li>
          <li>
            <button id="button_plus" type="button">
              +
            </button>
          </li>
        </ul>
      </div>
      <div style={{ display: 'flex' }}>{masterList}</div>
      <div style={{ display: 'flex' }}>{submasterList}</div>
      <div style={{ display: 'flex' }}>{techList}</div>
      <Link to="/list/:subject/:circle_id/members">
